...
interface StudentListProps {
  ...
  notesMap: Record<string, StudentNote>;
  date: string;
  school: string;
  class: string;
  ...
  onNotesClick: (studentId: string, note: StudentNote | null) => void;
}
          ...
          <li key={student.id} className="px-4 py-4 sm:px-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <button
                  onClick={() => onNotesClick(student.id, notesMap[student.id] || null)}
                  className={`p-1 rounded-full ${
                    notesMap[student.id]?.content 
                      ? notesMap[student.id]?.printInReport
