...
export default function PrintRegistersModal({ isOpen, onClose }: PrintRegistersModalProps) {
  ...
  const fetchRegisters = async () => {
    ...
    try {
      ...
      const dateA = (a.date as any).toDate();
      const dateB = (b.date as any).toDate();
                          ...
                          <tbody>
                            {register.students?.map((student: any, index: number) => (
                              <tr key={student.id} className="border-b border-gray-300">
                                ...
                                <td className="p-1 border-r border-gray-300 text-sm">
                                  {student.name}
                                  {student.note?.printInReport && student.note.content && (
                                    <div className="text-xs text-gray-600 mt-1">
                                      Nota: {student.note.content}
                                    </div>
                                  )}
                                </td>
